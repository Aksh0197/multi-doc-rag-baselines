# multi-doc-rag-baselines

Paper-grounded baseline project for multi-document RAG and top-k context selection.

This repository contains an offline, reproducible mini-benchmark for the two main ideas you highlighted:

- multi-document RAG
- top-k context selection in RAG

The implementation is grounded in your local `MultiDocRAG_Pipeline.docx` plus paper-level method summaries for:

- Lewis et al. RAG
- RankRAG
- Self-RAG
- RAGChecker
- MADAM-RAG

Because the environment did not have the HotpotQA/RAMDocs datasets or local dense-retrieval models preinstalled, this project uses a paper-grounded evaluation set instead of the full coursework datasets. The code is structured so you can swap in HotpotQA/RAMDocs later.

## What is implemented

- `tfidf`: sparse lexical retrieval baseline
- `bm25`: classic BM25 retrieval baseline
- `hybrid`: score interpolation between TF-IDF and BM25
- `hybrid_mmr`: hybrid retrieval plus MMR diversification for better multi-document top-k coverage

## Metrics

- `recall_at_1`, `recall_at_3`, `recall_at_5`
- `mrr`
- `multi_doc_hit_rate`
- `answer_exact_match`
- `answer_f1`

## Real Dataset Scaffold

The next-phase scaffold for real dataset runs lives under `src/rag_pipeline/`.

Use this when you have local copies of:

- `HotpotQA distractor`
- `RAMDocs`

Sample config:

- `configs/real_pipeline.example.json`

Run:

```bash
python3 src/run_real_pipeline.py configs/real_pipeline.example.json
```

The real pipeline now includes:

- retrieval
- reranking
- lightweight multi-agent debate
- HotpotQA supporting-fact evaluation

## Outputs

Results are written to the `output_dir` set in the config (e.g. `results/runtime/` for the example config):

- `summary_metrics.csv`
- `per_query_metrics.csv`
- `query_traces.json`
- `failed_query_traces.json`

These run outputs are gitignored since they're regenerated by re-running the pipeline.

## Repository layout

- `src/rag_pipeline/` — retrieval, reranking, debate, and evaluation modules
- `configs/` — pipeline run configs
- `docs/` — paper (`docs/paper/`), presentation (`docs/presentation/`), and explainer deep-dives (`docs/explainers/`)
- `results/` — gitignored; created locally when you run the pipeline
